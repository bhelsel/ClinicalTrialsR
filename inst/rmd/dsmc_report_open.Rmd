...
redcap <- 
  look_up_table %>%
  dplyr::filter(field == "intervention_status") %>%
  dplyr::rename(Status = id_name) %>%
  dplyr::select(-field) %>%
  merge(redcap, ., by.x = "intervention_status", by.y = "id_num", all = TRUE)

###### import ctcae categories and terms #######
load(system.file("extdata/ctcae.RData", package = "ClinicalTrialsR"))


ae_outcome_to_join <- look_up_table %>%
  dplyr::filter(field == "ae_outcome_2") %>%
  dplyr::select(-field)

ae_master <- redcap %>%
  dplyr::filter(!is.na(ae_date)) %>%
  dplyr::mutate(Related = dplyr::case_when(ae_related == 0 ~ "Not Related",
                                           ae_related == 2 ~ "Possibly Related",
                                           ae_related == 1 ~ "Definitely Related")) %>%
  dplyr::left_join(., ctcae[["Categories"]], by = c("ae_ctcae_category" = "ctcae_identifier")) %>%
  dplyr::left_join(., ctcae[["Terms"]], by = c("ae_ctcae_term" = "ctcae_identifier")) %>%
  dplyr::mutate(ctcae_terms = dplyr::case_when(grepl("Other",ctcae_term,ignore.case=T) & nchar(ae_ctcase_term_oth) <= 50 ~ ae_ctcase_term_oth, 
                                                    TRUE ~ ctcae_term)) %>%
  dplyr::mutate("SOC and Preferred Term" = dplyr::case_when(!grepl("Other", ctcae_term) ~ paste(ctcae_category,"-",ctcae_term),
                                                            TRUE ~ ctcae_term)) %>%
  dplyr::left_join(., ae_outcome_to_join, by = c('ae_outcome_2'='id_num')) %>%
  dplyr::rename("Outcome" = "id_name") %>%
  dplyr::mutate(Serious = dplyr::case_when(ae_was_ae_serious == 0 ~ "No",
                             ae_was_ae_serious == 1 ~ "Yes")) %>%
  dplyr::mutate(Severity = dplyr::case_when(ae_severity == 1 ~ "Grade 1; Mild",
  ...
  dplyr::mutate(ctcae_terms = dplyr::case_when(grepl("covid",ctcae_terms,ignore.case=TRUE) ~ "COVID-19",
                                               TRUE ~ ctcae_terms)) 
